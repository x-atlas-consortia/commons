...
key_callback = to_snake_case_upper  # The callback to apply on the dict key of the transform result 
...
When dealing with nested objects, one can grab values by providing `key_callback` and `val_callback` functions (both must be provided even if `as_arr=True`). 
...
UbkgSDK.ops(key_callback=key_callback, val_callback=val_callback, as_arr=True, cb=enum_val).assay_classes()
```
Will return the following transform result:
```
...
```
UbkgSDK.ops(key_callback=key_callback, val_callback=val_callback, as_data_dict=True).assay_classes()
